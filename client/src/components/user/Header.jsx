...
import { menuHide, menuShowHide } from '../../redux/features/user/menuSlice';
  ...
  return (
    <>
      <div onClick={() => dispatch(menuHide())}>
        <header className='p-3 flex justify-between 2xl:mx-48 xl:mx-20 lg:mx-10 md:mx-5 mx-2'>
          <Link to={'/'} className='hover:bg-transparent'>
            <div className="flex items-center gap-1">
              ...
              <span className='font-bold text-xl text-black sm:block hidden'>Querix</span>
            </div>
          </Link>
          <div className={`flex border items-center rounded-full ${searchBoxFocus && 'border-gray-500 border'} duration-1000 py-2 px-4 shadow-md shadow-gray-300 md:w-[400px] sm:w-[300px] xs:w-[200px] w-[150px]`}>
            <button className='bg-black text-white rounded-full p-2'>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
              </svg>
            </button>
            <div className='flex items-center px-2'><input onFocus={() => setSearchBoxFocus(true)} onBlur={() => setSearchBoxFocus(false)} className='outline-none  pl-1 md:w-80 sm:w-52 xs:w-28 w-14' id='searchBox' placeholder='Search' type="text" /></div>
          </div>
          <button className='flex border items-center gap-2 border-gray-300 rounded-full py-2 px-4' onClick={(e) => { e.stopPropagation(); dispatch(menuShowHide()) }}>
        ...
        <div className='relative 2xl:mr-44 xl:mr-28 '>
          <div className={`absolute z-10 -top-7 right-6 ${!menu ? 'hidden' : 'block'} text-base w-60 border-2 border-gray-100  rounded-lg  bg-gray-50 shadow-lg shadow-slate-300`}>
            {userData ?
              <>
                ...
                <Link to='/account' className='h-6 p-3 py-5  flex items-center hover:bg-gray-200 font-roboto '>Account</Link>
                <button className='h-6 p-3 py-5 flex items-center hover:bg-gray-200 font-roboto w-full' onClick={() => { dispatch(logout()) }}>Logout</button>
              </>
              :
              <>
