  ...
  return (
    <>
      <header className='p-3 flex justify-between mx-48'>
        ...
        <div className='flex border items-center gap-2 border-gray-300 rounded-full py-2 px-4' onClick={() => setMenuShow(!menuShow)}>
      ...
      <div className='relative mr-44'>
        <div className={`absolute z-10 -top-7 right-6 ${!menuShow ? 'hidden' : 'block'} text-base w-60 border-2 border-gray-100  rounded-lg  bg-gray-50 shadow-lg shadow-slate-300`}>
          {data ?
            <>
              <div className='h-6 p-3 py-5 mt-1 flex items-center hover:bg-gray-200 font-roboto font-semibold'>{data.name}</div>
              <div className='h-6 p-3 py-5 flex items-center hover:bg-gray-200 font-roboto ' onClick={logout}>Logout</div>
            </>
            :
            <>
