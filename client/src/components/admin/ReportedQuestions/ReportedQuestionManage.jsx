...
export default function ReportedQuestionManage() {

    const [reportedQuestions, setReportedQuestions] = useState([]);
    const [reportedQuestionReasons, setReportedQuestionReasons] = useState([]);
    ...
    //get all reported questions and update the reportedQuestions state
    useEffect(() => {
        try {
            ...
            axios.get("/admin/reported-questions-data", {
                headers: {
                ...
                setReportedQuestions(res.data.reportedQuestionsData);
            });
        } catch (err) {
            if (!err?.res) {
    ...
    useEffect(() => {
        console.log('Reported questions ', reportedQuestions);
    }, [reportedQuestions])

    const data = useMemo(() => reportedQuestions, [reportedQuestions]);


    //blocking & unblocking reported questions --SAME LOGIC AS QUESTION ACTIONS IN QUESTION MANAGE--
        ...
        try {
            ...
            if (userAction) {
                ...
                if (response.data.success) {
                    ...
                    const questionsResponse = await axios.get("/admin/reported-questions-data", {
                        headers: {
                            "Content-Type": "application/json",
                            Authorization: token,
                        },
                    ...
                    setReportedQuestions(questionsResponse.data.reportedQuestionsData);
                } else {
                ...
                wrapper2.innerHTML = `${action}ing questoin of <span style="font-weight: 500;">${userName}</span> has been cancelled.`;
    ...
    const handleViewReasons = (details) => {
        try {
            //Make a request to get the reasons take question id from details.id
            ...
            axios.get("/admin/reported-question-reasons", {
                params: { reportedQuestionId: details.id },
                headers: {
                    "Content-Type": "application/json",
                    Authorization: token,
                },
                ...
                setReportedQuestionReasons(res.data.reportedQuestionReasonsData);
            });
        } catch (err) {
            if (!err?.res) {
                message.error('Something went wrong.')
                ...
                localStorage.removeItem("admin");
                navigate('/admin/signin');
            } else if (err.response, data.message == "Invalid reported question.") {
                message.error("Can't get data. Invalid reported question.");
            } else {
                message.error('Something went wrong.')
            }
        }
    ...
    useEffect(() => {
        if (reportedQuestionReasons.length > 0) {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (props) => (
                <>
                    <div className="whitespace-nowrap overflow-ellipsis overflow-hidden mb-1">{props.row.original.question.title}</div>
                    <div className="text-sm font-normal whitespace-nowrap overflow-ellipsis overflow-hidden">{props.row.original.question.body}</div>
                </>
            )
        },
        {
        ...
        },
        {
            ...
            cell: (props) => props.row.original.answersCount,
        },
        {
            header: "Comments",
            accessorKey: "commentsCount",
            cell: (props) => props.row.original.commentsCount,
        },
        {
            header: "Reports",
                    ...
                    <dialog id={`dialog`} className='top-[35dvh] xl:left-[35dvw] lg:left-[25dvw] md:left-[23dvw] sm:left-[20dvw] xs:left-[20dvw] left-[5dvw] rounded-lg focus:outline-0 border border-gray-500/75 backdrop:backdrop-blur-[4px] backdrop:bg-gray-400 backdrop:bg-opacity-20'>
                        <div className='bg-gray-100 xs:w-[400px] sm:w-[400px] md:w-[500px] lg:w-[600px] xl:w-[650px] rounded-lg'>
                            <header className='bg-slate-200/50 flex flex-row justify-between items-center border-[0.5px] border-b-gray-400 px-7 py-4'>
                                <div className='text-xl font-bold'>Resons for reporting <span className='text-blue-600'>@{modalData?.author}'s</span> question</div>
                            ...
                            <div>
                                <div className='flex flex-col gap-5 h-72 my-4'>
                                    {reportedQuestionReasons.map((reason) => (
        ...
        },
        {
            ...
            cell: (props) => (
                <button
                    ...
                    onClick={() => {
                        console.log('clicked actions: ', props.row.original.question._id);
                        handleQuestionAction(props.row.original.question._id, props.row.original.author.userName, props.getValue().toLowerCase())
                    }}>
